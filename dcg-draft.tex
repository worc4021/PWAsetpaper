...
\begin{corollary}\label{thm:polytopic:set:not:p:convex}
  \revision{The polytopic parametric set-valued map $\mathcal W(p):=\{z: a_i z + b_i p\leq c_i \; \forall i\in\mathbb N_m\}$
    is  parametrically convex if and only if the matrix $B^T = [\begin{matrix} b_1^T & \cdots & b_m^T\end{matrix}]$ is such that $\rank(B) \leq 1$.}
%is not parametrically convex for any $B^T = [\begin{matrix} b_1^T & \cdots & b_m^T\end{matrix}]$ such that $\rank (B) >1$.}
\end{corollary}
%
\begin{proof}
\revision{First assume that $B=0$. In this case the graph $\mathscr G(\mathcal W)$ is constant for all $p$, i.e.
%
\begin{equation*}
	\mathscr G(\mathcal W) = \{(p,z):a_i z\leq c_i \; \forall i\in\mathbb N_m\},
\end{equation*}
%
which clearly satisfies the  conditions of Theorem~\ref{thm:p:convexity:graph} for parametric convexity.
%
...
\end{equation*}
%
where $\mathcal W(t)$ is empty for all $t > t^\ast$ for some $t^\ast$ and $\mathcal W(t)$ is a scaled version of $\{ z : Az\leq c\}$ for all $t \leq t^\ast$. Hence the conditions of Theorem~\ref{thm:p:convexity:graph} are again satisfied.}
%

\revision{Now assume that $\text{rank}(B)=r \geq 1$. In this case
%  $\image(B) = \{\gamma_1t_1+\cdots +\gamma_rt_r : t_1,\ldots,t_r \in \mathbb R\}$ for  some $\gamma_1, \ldots \gamma_r\in\mathbb{R}^m$.
  $\image(B) = \{\Gamma t : t \in \mathbb R^r\}$ for  some $\Gamma \in\mathbb{R}^{m\times r}$,
  %
  and $\mathscr{G}(\mathcal W)$ can be expressed
  % 
\begin{equation*}
  \mathscr G(\mathcal W) = \Bigl\{(t,z)\in\mathbb R^{r+n}:a_i z\leq c_i-\sum_{j=1}^r \gamma_{i,j} t_j \; \forall i\in\mathbb N_m\Bigr\},
\end{equation*}
%
where $\gamma_{i,j}$ is the $(i,j)$th element of $\Gamma$.
%
Whenever $\text{rank}(B)\geq 2$ therefore, the graph $\mathscr G(\mathcal W)$ is given by a convex polyhedron that has vertices in the parameter space, which implies that the graph is locally strictly convex and $\mathcal W(p)$ therefore violates Theorem~\ref{thm:p:convexity:graph}.}
\qed
\end{proof}
%
