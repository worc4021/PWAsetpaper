...
\begin{abstract}
The Pontryagin set difference is a commonly used operation between two polytopes to calculate a set of safe operation.
%
We suggest to use a particular class of set-valued maps as the subtrahend, for this we introduce the property of parametric convexity and present some of its analytic framework.
%
In order to guarantee the convexity of the difference of a convex minuend and a set-valued map it proves necessary and sufficient to have the parametrically convexity property of the set-valued map.
%
For generic piecewise polyhedral set-valued maps we prove that parametric convexity is equivalent to a constant combinatorial structure of all its realisations.
%
We present a computational way of verifying parametric convexity of a piecewise polyhedral set-valued map as well as a way to calculate the parametric Pontryagin difference between a polyhedron and a piecewise polyhedral set-valued map.
%
...
%
Furthermore, since $\lambda(p_1,z_1)+(1-\lambda)(p_2,z_2)\not\in \text{int}(\mathscr G(\mathcal W))$ and both $(p_1,z_1)$ and $(p_2,z_2)$ die on the boundary either $z_1-z_2$ or $z_2-z_1$ is outward facing for both $\mathcal W(p_1)$ and $\mathcal W(p_2)$, therefore either $z_1-z_2$ or $z_2-z_1$ is in $\mathcal N\mathcal W(p_1,z_1)$ and $\mathcal N\mathcal W(p_2,z_2)$ and hence in their intersection.
