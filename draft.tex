...
%
When the Pontryagin differnece has to be performed explicitly by numeric algorithms it is almost imperative that the respective sets are polytopic, since it is not possible to operate on general sets explicitly.
%
...
%
The sets on which these operations can be performed are fixed predetermined polytopes, due to their physical interpretation safe sets, it can be desirable to extend the class of sets on which these operations can be performed.
%
...
%
However, computational aspects of operations on set-valued maps are largely unknown, we refer to~\cite{Finzel:2000} for some results for polyhedral set-valued maps.
%
...
%
The set-valued maps we consider are parametrised polytopes with the so called \emph{parametric convexity} property which allows us to guarantee that the \emph{parametric Pontryagin difference} is convex for all convex minuend sets.
%
...
%
We denote index sets by $\mathcal A$ or $\mathcal I$, by $f\vert_\Omega$ we mean $f$ evaluated only on values in $\Omega$.
%
The standard Minkowski sum~\cite{Minkowski:1911} of the sets $X_1,X_2$ is denoted by $X_1\oplus X_2$
%
...
The particular property we require is \emph{simplicity}, for each $y\in Y$ the set $f(y)$ is a polytope and each of its vertices~$v(y)$ is supported by a minimal number of hyperplanes, i.e. there exists linear system defining the set~$f(y) = \{z:A(y)z\leq b(y)\}$ and each vertex $v(y)$ is defined by $n$ hyperplanes $A_{\mathcal I}(y)v(y)=b_{\mathcal I}(y)$ and $A_{\bar{\mathcal I}}(y)v(y)<b_{\bar{\mathcal I}}(y)$.
%
...
%
This property is then used to demonstrate the convexity of a generalised Pontryagin difference (see e.g.~\cite{Hadwiger:1950,blanchini:2007}). 
%
...
%
A similar definition was given in~\cite{Hadwiger:1957} for scalar families of sets ($d=1$), however the results presented are of no particular use for us here.
...
%
We will illustrate how we exploit this property for our benefit in the Section~\ref{sec:computational:methods} and~\ref{sec:numerical:examples}.
%
...
%
For notational convenience let $d_i(p) = \max_k\{b_{i,k}+c_{i,k}p\}$ denote the right hand side of~\eqref{eq:definition:PWA:polytopic:set:general}, i.e. $\mathcal W(p) = \{z\in\mathbb R^n: a_i z\leq d_i(p)\;\forall i\in\{1,\dots,m\}\}$.
%
...
\end{split}\end{equation}
%
The term~$(\dagger)$ is a multi parametric linear program (mpLP), we less interested in the objective value of~$(\dagger)$ but rather in the shape of its solution over~$p$ since it defines the shape of the set of points which can only just be taken to $P_i$ for an extremal value of $w\in\mathcal W(p)$.
%
...
\end{equation}
%
defines all vertices $\mathcal W(p) = \conv\{w_1(t(p)),\dots,w_N(t(p))\}$ for a element-wise convex piecewise affine function $t(p)$.
%
It is easy to see that the choice $t_i(p)=d_i(p)$ again leads us to the epigraph~$\epi(d_i)$ and therefore that the maximisation~$(\dagger)$ becomes
%
...
\end{equation}
%
but this is the definition of the projection onto $\mathbb R^d$, $\pi_d(Q)$.
%
...
%
Instead of using~$\tilde P_i$ which are points~$p$ that can be taken to $P_i$ for a feasible $w\in\mathcal W(p)$ and the values of the~$d_i(p)$ and so on, we obtain all feasible~$p$ such that a feasible $w\in\mathcal W(p)$ exists such that $p+w$lies on the boundary of $S$.
%
\begin{equation}
...
\end{equation}
%
The set~$L$ contains all such $p$ for which there exists a feasible $w\in\mathcal W(p)$ such that $p+w\in S$ and hence the projection $\pi_d(L)=S\ominus\mathcal W(S)$, which does not require a vertex enumeration.
%
%
%
%
...
%
%
In this paper we presented a way to extend the scope of the Pontryagin difference to a wider class of sets.
%
We introduced the framework of parametric convexity and some of its analysis, which proved to be essential to prove the convexity of the parametric Pontryagin difference.
%
Furthermore we provided the link required to make the parametric Pontryagin difference computationally accessible for polytopic sets, it relied fully on the combinatorial nature of the subtrahend set-valued map.
%
Using two simple examples we illustrated the potency of the proposed methodology.

The parametric Pontryagin difference has been applied successfully in a robust control environment~\cite{Schaich:2015,Schaich:2015a} and will continue to extend its scope of application.
